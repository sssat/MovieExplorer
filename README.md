...
KOBIS와 TMDB의 영화 데이터를 결합하고 MSSQL에 저장·분석하는 C# WPF 데스크톱 애플리케이션입니다.

KOBIS가 제공하는 국내 박스오피스 순위·관객 데이터에 TMDB의 포스터·줄거리·평점·장르 정보를 연결합니다. 외부 API 결과를 단순히 화면에 표시하는 데 그치지 않고, 정규화된 데이터베이스에 저장한 뒤 검색·통계·개인 기록 기능에 활용하는 것을 목표로 합니다.
...
| 데이터 결합 | KOBIS 영화에 TMDB 포스터·줄거리·평점·장르 연결 | 구현 완료 |
| 상영 중 | TMDB 한국 지역 상영 영화 조회 | 구현 완료 |
...
4. 관심 영화·개인 감상평 CRUD 구현
5. 개봉 예정·지난 영화 및 상세 화면 구현
6. SQL 기반 박스오피스 통계 구현
7. 최소 데이터 관리 화면 구현
8. 테스트, 쿼리 성능 개선 및 문서화

## 포트폴리오 핵심 내용

외부 API 데이터를 단순 출력하지 않고 MSSQL에 정규화하여 저장하고, 서로 다른 KOBIS·TMDB 데이터를 하나의 영화 정보로 통합하는 과정을 구현합니다. 데이터 동기화 시 트랜잭션과 제약조건으로 정합성을 유지하며, 저장 프로시저·JOIN·CTE·윈도우 함수로 박스오피스 통계를 제공합니다.
